...
function isOnboardingPropertyType(
  value: string,
): value is OnboardingPropertyType {
...
export default function OnboardingPage() {
  ...
  const [rooms, setRooms] = useState([
    { roomNumber: "", type: "SINGLE", bedCount: 1, monthlyRent: "" },
  ]);
  ...
  const handleRoomsSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err.message ||
          "Failed to create rooms. You can skip and do this later.",
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-2xl">
        <div className="flex justify-center mb-6">
          <div className="flex items-center space-x-4">
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${step >= 1 ? "bg-primary text-white" : "bg-gray-200 text-gray-500"}`}
            >
              1
            </div>
            <div
            ...
            ></div>
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${step >= 2 ? "bg-primary text-white" : "bg-gray-200 text-gray-500"}`}
            >
              2
            </div>
            <div
            ...
            ></div>
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${step >= 3 ? "bg-primary text-white" : "bg-gray-200 text-gray-500"}`}
            >
              3
            </div>
          </div>
            ...
            <form className="space-y-6" onSubmit={handleProfileSubmit}>
              <div className="text-center mb-8">
                ...
                <p className="mt-2 text-sm text-gray-500">
                  How should we address you and your business?
                </p>
              </div>
          ...
          {step === 2 && (
            <form className="space-y-6" onSubmit={handlePropertySubmit}>
              <div className="text-center mb-8">
                ...
                <p className="mt-2 text-sm text-gray-500">
                  Set up the building where your tenants reside.
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label
                    htmlFor="onboarding-property-type"
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 py-2 text-sm text-gray-600 hover:bg-gray-50 rounded-lg transition-colors border border-gray-200"
                >
                  Skip for now
                </button>
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-[2] flex justify-center items-center py-2.5 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-primary hover:bg-primary/90 transition-colors"
                >
                  {loading ? (
                    "Creating..."
                  ) : (
                    <>
                      <span className="mr-2">Create Property</span>{" "}
                      <ChevronRight size={16} />
                    </>
                  )}
                </button>
          ...
          {step === 3 && (
            <form className="space-y-6" onSubmit={handleRoomsSubmit}>
              <div className="text-center mb-8">
                ...
                <p className="mt-2 text-sm text-gray-500">
                  Quickly add a few rooms to get started.
                </p>
              </div>
                  ...
                  <div key={i} className="flex gap-3 items-end">
                    <div className="flex-1">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const nm = [...rooms];
                          nm[i].roomNumber = e.target.value;
                          setRooms(nm);
                        }}
                      />
                    </div>
                    ...
                    <div className="flex-[1.5]">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const nm = [...rooms];
                          nm[i].monthlyRent = e.target.value;
                          setRooms(nm);
                        }}
                      />
                    </div>
          ...
          {step === 4 && (
            <div className="text-center py-12">
              ...
              <p className="mt-2 text-sm text-gray-600 mb-8">
                Your profile, property, and initial rooms have been created.
                Let's head to your dashboard!
              </p>
              <button
                onClick={skipToDashboard}
                className="w-full justify-center items-center py-3 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-primary hover:bg-primary/90 transition-colors"
              >
                Go to Dashboard
              </button>
            </div>
